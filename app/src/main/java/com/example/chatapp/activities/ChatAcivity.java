...
import com.example.chatapp.databinding.ActivityChatAcivityBinding;
...
public class ChatAcivity extends BaseActivities {

    private ActivityChatAcivityBinding binding;
    ...
    private void sendMessage(){
        ...
        if(  !TextUtils.isEmpty(binding.inputMessage.getText().toString().trim())) {
    ...
    private void listAvailabilityOfReceiver()
    {
        database.collection(Constants.KEY_COLLECTION_USER).document(
                receiverUser.id
        ).addSnapshotListener(ChatAcivity.this,((value, error) -> {
            if(error !=null){
                return;
            }
            if(value !=null){
